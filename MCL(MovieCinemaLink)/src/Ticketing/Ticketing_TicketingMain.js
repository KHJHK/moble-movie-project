...
const Ticketing_TicketingMain = () => {
  ...
  const [movie_id, setMovieId] = useState(0); // 영화 선택시 id값
  const [movie_RG, setMovieRg] = useState(0); // 지역 눌렀을 때 텍스트 값
  const [movie_cn, setMoviecn] = useState(0); //극장 눌렀을 때 텍스트 값
  const [movie_Tm, setMovieTm] = useState(0); //시간 눌렀을 때 텍스트 값
  ...
  useEffect(() => {
    axios.get(`http://localhost/ticketing/selectMovie`).then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost/ticketing/selectLocation?movie_id=${movie_id}`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost/ticketing/selectCinema?movie_id=${movie_id}&cinema_location=${movie_RG}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost/ticketing/selectDate?movie_id=${movie_id}&cinema_name=${movie_cn}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost/ticketing/selectTime?movie_id=${movie_id}&cinema_name=${movie_cn}&schedule_date=${movie_Tm}`
      )
      .then((response) => {
                ...
                <div>
                  <ul>
                    <li
                      className="SelectMV"
                      onClick={(e) => {
                        storeMovieId(item.movie_id), movieClick(e);
                      }}
                    >
                      {item.movie_name}
                    </li>
                ...
                <div>
                  <ul>
                    <li
                      className="SelectTI"
                      onClick={(e) => {
                        storeMovieId5(e, item.schedule_id), TimeClick(e);
                      }}
                    >
